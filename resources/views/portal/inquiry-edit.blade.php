...
<head>
  ...
  <title>Edit Inquiry | 3DHub Data Portal</title>
  ...
  <div class="pq-hero">
    <div class="container">
      <div class="d-flex justify-content-between align-items-center flex-wrap gap-3" style="position:relative;z-index:1">
        <div>
          <h1><i class="bx bx-edit me-2"></i>Edit Inquiry</h1>
          <p>Modify your required output formats and adjust your inquiry area on the map</p>
        </div>
    ...
    <div class="container">
      <div class="card shadow-sm border">
        <div class="card-header border-bottom d-flex justify-content-between align-items-center py-4 px-4 px-md-5">
          <h4 class="m-0 fw-bold"><i class="bx bx-edit me-2 text-primary"></i>Edit Inquiry</h4>
          <span class="badge bg-label-warning">Edit Inquiry</span>
        </div>
        <div class="card-body p-4 p-md-5">
          ...
          <div id="successAlert" class="alert alert-success d-none mb-4" role="alert">
            <div class="d-flex">
              ...
              <div>
                <h6 class="alert-heading mb-1 fw-bold">Inquiry Updated Successfully!</h6>
                <span>Your inquiry request has been updated. We will review the details and get back to you shortly.</span>
              </div>
            </div>
          </div>
          ...
          <form id="inquiryForm" novalidate>
              ...
              <div class="col-lg-5">
                ...
                <div class="mb-4">
                  ...
                  <div class="input-group input-group-merge">
                    ...
                    <input type="text" id="inquiry_id" name="inquiry_id" class="form-control fw-bold text-primary" value="{{ $inquiry->inquiry_id }}" readonly style="background-color: var(--bs-tertiary-bg);">
                  </div>
                  <div class="form-text">The unique ID for this inquiry request. It cannot be changed.</div>
                </div>
                ...
                <div class="mb-4">
                  ...
                  <div class="form-check format-option-wrapper">
                    <input class="form-check-input" type="checkbox" name="output_categories[]" id="catDSM" value="DSM" {{ in_array('DSM', $inquiry->output_categories ?? []) ? 'checked' : '' }}>
                  ...
                  <div class="form-check format-option-wrapper">
                    <input class="form-check-input" type="checkbox" name="output_categories[]" id="cat3DGS" value="3DGS" {{ in_array('3DGS', $inquiry->output_categories ?? []) ? 'checked' : '' }}>
                  ...
                  <div class="form-check format-option-wrapper">
                    <input class="form-check-input" type="checkbox" name="output_categories[]" id="catOrthophoto" value="Orthophoto" {{ in_array('Orthophoto', $inquiry->output_categories ?? []) ? 'checked' : '' }}>
            ...
            <div class="row mt-4">
              <div class="col-12 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                <a href="{{ route('inquiry.my') }}" class="btn btn-outline-secondary px-4">Cancel</a>
                ...
                <div class="d-flex flex-column flex-md-row align-items-md-center gap-2 flex-grow-1 justify-content-end">
                  ...
                  <!-- Update Inquiry Button on the bottom right -->
                  <button type="submit" id="btnSubmitQuotation" class="btn btn-primary px-5 fw-bold">
                    <i class="bx bx-save me-1"></i> Update Inquiry
                  </button>
                </div>
              </div>
            </div>
  ...
  <script>
    (function() {
      ...
      getViewer(function(viewer) {
        ...
        // Pre-load coordinates and selection on load if editing
        (function() {
          var initialCoords = @json($inquiry->area_coordinates);
          if (initialCoords && initialCoords.coordinates && initialCoords.coordinates[0]) {
            var pts = initialCoords.coordinates[0];
            // If the last point closes the polygon, remove it for editing handles
            if (pts.length > 1) {
              var first = pts[0];
            ...
            polygonPoints = pts.map(function(pt) {
              return C.Cartesian3.fromDegrees(pt[0], pt[1]);
            });
            
            // Trigger polygon rendering and edit handles setup!
            if (polygonPoints.length >= 3) {
              completeDrawing();
              
              // Fly camera to the loaded polygon
              setTimeout(function() {
                var boundingSphere = C.BoundingSphere.fromPoints(polygonPoints);
                viewer.camera.flyToBoundingSphere(boundingSphere, {
                  duration: 1.5,
                  offset: new C.HeadingPitchRange(0, C.Math.toRadians(-90), boundingSphere.radius * 2.5)
                });
              }, 500);
            }
          }
      ...
      document.getElementById('inquiryForm').addEventListener('submit', async function(e) {
        ...
        try {
          var res = await fetch('{{ route('inquiry.update', $inquiry->id) }}', {
            method: 'POST',
            headers: {
