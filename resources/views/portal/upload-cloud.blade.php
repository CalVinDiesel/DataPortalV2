  ...
  <script>
    ...
    document.getElementById('cloudForm').addEventListener('submit', async function(e) {
      ...
      if (currentProvider === 'google_drive') {
          ...
          else if (oSize.includes('.') || parseInt(oSize, 10) <= 0 || isNaN(parseInt(oSize, 10))) {
              return alert('Invalid Size: Total Size (Bytes) must be a whole number (e.g. 5368709120).\n\nDo NOT enter decimal values like "1.1" - please enter the exact byte count.');
          }
      ...
      if (currentProvider === 'google_drive') {
        ...
        // The field label says "Bytes", so only whole numbers are valid.
        payload.onedriveSize = parseInt(oSize, 10) || 0;
        payload.onedriveCount = parseInt(oCount, 10) || 0;
