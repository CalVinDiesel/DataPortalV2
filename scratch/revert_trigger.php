...
BEGIN
    IF (NEW.role IN ('trusted', 'admin', 'superadmin', 'registered') AND NEW.is_active = true) THEN
        -- 🚀 REVERT TO PLURAL PATHING (v168)
        IF (NEW.role IN ('admin', 'superadmin')) THEN
            v_home_dir := '/home/tiquan/deliveries/' || NEW.sftp_username;
        ELSE
...
// Update existing SFTP users
DB::statement("
    UPDATE public.users u
    SET home_dir = CASE 
        WHEN du.role IN ('admin', 'superadmin') THEN '/home/tiquan/deliveries/' || du.sftp_username
        ELSE '/home/tiquan/uploads/' || du.sftp_username
    END
